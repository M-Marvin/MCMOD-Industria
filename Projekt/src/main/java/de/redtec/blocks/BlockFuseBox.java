...
public class BlockFuseBox extends BlockContainerBase implements IElectricConnective, IAdvancedBlockInfo {
	...
	public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
		...
		if (te instanceof TileEntityFuseBox && handIn == Hand.MAIN_HAND) {

			ItemStack heldStack = player.getHeldItemMainhand();
			ItemStack fuseStack = ((TileEntityFuseBox) te).getFuse();
			
			if ((heldStack.isEmpty() || !fuseStack.isEmpty()) && player.isSneaking()) {
				
				ItemStack removeStack = ((TileEntityFuseBox) te).removeFuse();
				...
				boolean success = ((TileEntityFuseBox) te).insertFuse(heldStack);
				...
				if (((TileEntityFuseBox) te).canSwitch()) {
	...
	public boolean beforNetworkChanges(World world, BlockPos pos, BlockState state, ElectricityNetwork network, int lap) {
		...
		if (te instanceof TileEntityFuseBox) {
			maxCurrent = ((TileEntityFuseBox) te).getMaxCurrent();
		}
