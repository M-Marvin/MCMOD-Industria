...
public class BlockSignalProcessorContact extends BlockContainerBase implements ISignalConnective, IWaterLoggable {
	
	public static final DirectionProperty FACING = BlockStateProperties.FACING;
	public static final BooleanProperty OPEN = BooleanProperty.create("open");
	public static final BooleanProperty WATERLOGGED = BlockStateProperties.WATERLOGGED;
	
	public BlockSignalProcessorContact() {
	...
	public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
		...
		if (handIn == Hand.MAIN_HAND) {
			...
			if (player.isSneaking()) {
				...
				if (tileEntity instanceof TileEntitySignalProcessorContact) ((TileEntitySignalProcessorContact) tileEntity).getVariables().clear();
				...
				if (tileEntity instanceof TileEntitySignalProcessorContact) {

					if (!heldStack.isEmpty() && !((TileEntitySignalProcessorContact) tileEntity).hasProcessor()) {
						
						ItemStack stackToSet = heldStack.copy();
						stackToSet.setCount(1);
						
						boolean flag = ((TileEntitySignalProcessorContact) tileEntity).setProcessorStack(stackToSet);
						if (flag) {
							heldStack.shrink(1);
							flag1 = true;
						}
						
					} else if (((TileEntitySignalProcessorContact) tileEntity).hasProcessor()) {
						
						ItemStack processorStack = ((TileEntitySignalProcessorContact) tileEntity).getProcessorStack();
						...
						if (ItemStackHelper.isItemStackItemEqual(processorStack, heldStack, true) && (heldStack.getCount() < heldStack.getMaxStackSize() || heldStack.isEmpty())) {
							...
							((TileEntitySignalProcessorContact) tileEntity).removeProcessor();
				...
				if (tileEntity instanceof TileEntitySignalProcessorContact) {
					...
					if (((TileEntitySignalProcessorContact) tileEntity).hasProcessor()) {
	...
	public void onBlockHarvested(World worldIn, BlockPos pos, BlockState state, PlayerEntity player) {
		...
		if (tileEntity instanceof TileEntitySignalProcessorContact) {
			...
			if (((TileEntitySignalProcessorContact) tileEntity).hasProcessor()) {
				...
				ItemStack drop = ((TileEntitySignalProcessorContact) tileEntity).getProcessorStack();
	...
	public void animateTick(BlockState stateIn, World worldIn, BlockPos pos, Random rand) {
		...
		if (rand.nextInt(5) == 0) {
			...
			if (tileEntity instanceof TileEntitySignalProcessorContact ? ((TileEntitySignalProcessorContact) tileEntity).hasProcessor() : false) {
	...
	public TileEntity createNewTileEntity(IBlockReader worldIn) {
		return new TileEntitySignalProcessorContact();
	}
	...
	public void onReciveSignal(World worldIn, BlockPos pos, RedstoneControlSignal signal, Direction side) {
		...
		if (tileEntity instanceof TileEntitySignalProcessorContact) {
			...
			((TileEntitySignalProcessorContact) tileEntity).setInput(signal.getChanelItem(), signal.isPowered());
