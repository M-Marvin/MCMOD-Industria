...
public interface IElectricConduit extends IElectric<PlacedConduit, ConduitPos> {
	...
	default ConnectionPoint[] getConnections(Level level, ConduitPos pos, PlacedConduit instance) {
		...
		ConnectionPoint[] points1 = state1.getBlock() instanceof IConduitConnector ? ((IConduitConnector)state1).getConnectionPoints(instance.getNodeA(), state1) : null;
		ConnectionPoint[] points2 = state2.getBlock() instanceof IConduitConnector ? ((IConduitConnector)state2).getConnectionPoints(instance.getNodeB(), state2) : null;
