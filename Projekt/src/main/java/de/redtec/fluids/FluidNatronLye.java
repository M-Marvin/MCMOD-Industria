...
public abstract class FluidNatronLye extends FlowingFluid {
	...
	public Item getFilledBucket() {
		return RedTec.natron_lye_bucket;
	}
	...
	protected BlockState getBlockState(FluidState state) {
		return RedTec.natron_lye.getDefaultState().with(BlockModFlowingFluid.LEVEL, getLevelFromState(state));
	}
	
	@Override
	protected FluidAttributes createAttributes() {
		return FluidAttributes.builder(
				new ResourceLocation(RedTec.MODID, "block/natron_lye_still"), 
				new ResourceLocation(RedTec.MODID, "block/natron_lye_flow"))
					.overlay(new ResourceLocation(RedTec.MODID, "block/natron_lye_overlay"))
					.build(this);
	}
		...
		for (Direction d : Direction.values()) {
			...
			if (random.nextInt(resistance * 4) == 0 && resistance <= 1.5F) {
				...
				if (resistance < 35 && !state1.isAir() && !(state1.getBlock() instanceof FlowingFluidBlock)) {
					world.setBlockState(pos.offset(d), d == Direction.DOWN ? ModFluids.FLOWING_NATRON_LYE.getDefaultState().getBlockState() : Blocks.AIR.getDefaultState());
