...
public class BlockRadialConector extends BlockContainerBase implements IAdvancedStickyBlock {
	...
	public boolean addBlocksToMove(AdvancedPistonBlockStructureHelper pistonStructureHelper, BlockPos pos, BlockState state, World world) {
		...
		if (state.get(LOCKED) && tileEntity instanceof TileEntityLockedCompositeBlock) {
			connectedBlocks = ((TileEntityLockedCompositeBlock) tileEntity).getConnectedBlocks();
		} else {
	...
	public void setLocked(boolean locked, World world, BlockPos pos, BlockState state) {
		...
		if (locked) {
			...
			if (blocks.size() > 0) {
				...
				TileEntityLockedCompositeBlock tileEntity = new TileEntityLockedCompositeBlock();
	...
	public TileEntity createNewTileEntity(IBlockReader worldIn) {
		return new TileEntityLockedCompositeBlock();
	}
