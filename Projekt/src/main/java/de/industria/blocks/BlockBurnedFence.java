...
public class BlockBurnedFence extends FenceBlock {
	...
	public BlockBurnedFence() {
		super(Properties.create(Material.WOOD).hardnessAndResistance(1F, 0.3F).sound(SoundType.LADDER).harvestTool(BlockBase.getDefaultToolType(Material.WOOD)).setRequiresTool());
		this.setDefaultState(this.stateContainer.getBaseState().with(NORTH, Boolean.valueOf(false)).with(EAST, Boolean.valueOf(false)).with(SOUTH, Boolean.valueOf(false)).with(WEST, Boolean.valueOf(false)).with(WATERLOGGED, Boolean.valueOf(false)).with(PERSISTANT, true).with(DISTANCE, 1));
